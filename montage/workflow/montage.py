...
config = Config(
        executors=[TaskVineExecutor(
            manager_config=TaskVineManagerConfig(port=0, project_name="parsl_vine_montage"),
...
def create_input_metadata_table(inputs=[], outputs=[], stdout='cimt.stdout', stderr='cimt.stderr', parsl_resource_specification={"cores":12, "disk":10000}):
    return f"mImgtbl . {outputs[0]} -t {inputs[0]}"    

@bash_app
#def create_repro_metadata_table(inputs=[], outputs=[]):
def create_repro_metadata_table(inputs=[], outputs=[], stdout='cpromdt.stdout', stderr='cpromdt.stderr', parsl_resource_specification={"cores":12, "disk":10000}):
    return f"mImgtbl . {outputs[0]} -t {inputs[-1]}"    

@bash_app
#def create_fits_header(inputs=[],outputs=[]):
def create_fits_header(inputs=[],outputs=[], stdout='cfhdr.stdout', stderr='cfhdr.stderr', parsl_resource_specification={"cores":12, "disk":10000}):
...
def project_input_images(inputs=[],outputs=[], stdout='projii.stdout', stderr='projii.stderr', parsl_resource_specification={"cores":12, "disk":10000}):
    return f"mProjExec {inputs[0]} {inputs[1]} . {outputs[0]}"

@bash_app
#def analyze_overlap(inputs=[],outputs=[]):
def analyze_overlap(inputs=[],outputs=[], stdout='aolap.stdout',stderr='aolap.stderr', parsl_resource_specification={"cores":12, "disk":10000}):
    return f"mOverlaps {inputs[0]} {outputs[0]}"

@bash_app
#def create_diffs(inputs=[],outputs=[], parsl_resource_specification={"cores":8, "disk":1000}):
def create_diffs(uid, inputs=[],outputs=[], stdout='cdiff.stdout', stderr='cdiff.stderr', parsl_resource_specification={"cores":12, "disk":10000}):
    return f"mDiffExec -p . {inputs[0]} {inputs[1]} . ; rename diff. {uid}-diff. diff.*"

@bash_app
#def fit_diffs(inputs=[],outputs=[]):
def fit_diffs(inputs=[],outputs=[], stdout='fdiff.stdout', stderr='fdiff.stderr', parsl_resource_specification={"cores":12, "disk":10000}):
    return f"mFitExec {inputs[0]} {outputs[0]} ."

@bash_app
#def compute_corrections(inputs=[],outputs=[]):
def compute_corrections(inputs=[],outputs=[], stdout='compcorr.stdout', stderr='compcorr.stderr', parsl_resource_specification={'cores':12, 'disk':10000}):
    return f"mBgModel {inputs[0]} {inputs[1]} {outputs[0]}"

@bash_app
#def apply_corrections(inputs=[],outputs=[]):
def apply_corrections(inputs=[],outputs=[], stdout='acorr.stdout', stderr='acorr.stderr', parsl_resource_specification={'cores':12, 'disk':10000}):
    return f"mBgExec {inputs[0]} {inputs[1]} . ; echo dummy > applied_dummy"

@bash_app
#def add_to_mosaic(inputs=[],outputs=[]):
def add_to_mosaic(inputs=[],outputs=[], stdout='atm.stdout', stderr='atm.stderr', parsl_resource_specification={"cores":12, "disk":10000}):
    return f"mAdd {inputs[0]} {inputs[1]} {outputs[0]}"

@bash_app
#def create_png(inputs=[],outputs=[]):
def create_png(inputs=[],outputs=[], stdout='cpng.stdout', stderr='cpg.stderr', parsl_resource_specification={'cores':12, 'disk':10000}):
...
with parsl.load(config) as conf:
    ...
    for uid in uid_set:
        #kimages
        f_input_metadata = File(f"{uid}-Kimages.tbl")
        f_fits_header = File(f"{uid}-Ktemplate.hdr")
        f_imglist = File(f"{uid}-imglist")
        f_repro_imglist = File(f"{uid}-repro_imglist")

        #kprojdir
        f_input_projection_stats = File(f"{uid}-Kstats.tbl")
        f_overlap_diffs = File(f"{uid}-diffs.tbl")
        f_repro_metadata = File(f"{uid}-images.tbl")
        
        #diffdir
        f_fits_table = File(f"{uid}-fits.tbl")
        f_corrections_table = File(f"{uid}-corrections.tbl")
        f_mosaic = File(f"{uid}-m17.fits")
        f_png = File(f"{uid}-m17.png")
